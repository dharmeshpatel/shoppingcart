...
#        self.assertRaises(ValueError, cart_obj.add_item(product=1, quantity=1))
    ...
    def test_add_update_remove_options(self):
        ...
        self.assertEqual([{'product': item.product, 'quantity': item.quantity, 'price': item.price, 'options': item.get_options()} for item in cart_obj.get_items()], [{'product': 1, 'price': 10.0, 'quantity': 1}, {'product': 2, 'price': 12.25, 'options': {'color': {'red': {'price': 10.0}}}, 'quantity': 2.5}])
        self.assertEqual(cart_obj.count(), 3.5)
        self.assertEqual('%s %s'%(cart_obj.sub_total(), cart_obj.currency_symbol), '50.63 €')
        self.assertEqual('%s %s'%(cart_obj.total_untaxed_amount(), cart_obj.currency_symbol), '50.63 €')
        self.assertEqual('%s %s'%(cart_obj.total_tax(), cart_obj.currency_symbol), '0.0 €')
        self.assertEqual('%s %s'%(cart_obj.total(), cart_obj.currency_symbol), '50.63 €')
